% plot_figure_6.m
%
% Script to makes figure 6 for publication, about photometry
%
% MATLAB R2021b
...
params = struct('trace_type', 'Z Score Heat Duration');

groups = {{'M5','M4','M3','M2','M1'};{'F5','F4','F3','F2','F1'}};
...
plot_path  = fullfile(paths.dropbox,'plots','photometry','activity_behavior','all');
...
layout_main = tiledlayout(1,2,'TileSpacing','compact','Padding','none');

% Prepare figure C_1
tile_M = nexttile(layout_main,1); axis off;
layout_M = tiledlayout(layout_main, numel(groups{1}), 1,'TileSpacing','tight','Padding','tight','TileIndexing','columnmajor');
layout_M.Layout.Tile = 1;

% Prepare figure C_2
tile_F = nexttile(layout_main,1); axis off;
layout_F = tiledlayout(layout_main, numel(groups{1}), 1,'TileSpacing','tight','Padding','tight','TileIndexing','columnmajor');
layout_F.Layout.Tile = 2;


%% C - 1

plot_traces(params, groups{1}, layout_M, paths, '')



%% C - 2

plot_traces(params, groups{2}, layout_F, paths, '')
...
for mouse = 1:numel(mouse_ids)
    ...
    plot(trace_table(trace_2_behave,1), trace_table(trace_2_behave,2), 'o', 'MarkerFaceColor','c','MarkerEdgeColor','none','DisplayName','Hyperalgesia','MarkerSize',5) % Plot pain days
    xline(0,'Tag','trigger')
