...
date_str = '08232022';               % Date of recording
cond_type = 'pre';                   % Pain type ('pre','cfa','cci')
cond_day  = 0;                       % Day since pain type (0 for 'pre')
...
fig_params = struct('Position',[0 0 4 4], 'Units','Inches', 'WindowStyle','normal', 'FontSize', 10);
...
for mouse = 1:numel(mouse_ids)
    ...
    for i = 1:size(condition_table,1)
        ...
        plot_path  = fullfile(paths.dropbox,'plots',findPlotFolder,date,'photometry',date_str, 'recording', folder_name);
...
%         end
        ...
        ylabel('Z-score')
        H.mainLine.DisplayName = ['Averaged over ',num2str(sum(good_trials)),' trials'];
