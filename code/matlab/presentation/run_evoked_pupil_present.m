% run_evoked_pupil_present.m
%
% Script that plots pupil comparing conditions
%
% MATLAB R2021b
...
paths = rmfield(paths,'neural'); % Delete pupil field so that loadData knows to skip it
...
for mouse = 1:numel(mouse_ids)
    ...
    for d = 1:numel(dates)
        ...
        plot_path  = fullfile(paths.dropbox,'plots',findPlotFolder,date,'evoked', params_folder, mouse_id, date_str, file_num);
        ...
        plot_data = step_plot(mean(exper_data.(exper_cond).trigger_data.pupil.data,1,'omitnan'));
        plot_CI = step_plot(calculateCR(exper_data.(exper_cond).trigger_data.pupil.data')');
        thresh = max(calculateCI(reshape(exper_data.(exper_cond).trigger_data.pupil.data(:,plot_time < 0),1,[])));
        H = shadedErrorBar(plot_time(2:end), plot_data(1:end-1), plot_CI(1:end-1),'lineProps',{'k'});
        H.mainLine.DisplayName = 'Pre Conditioning';
        h(1) = H.mainLine;
        yline(thresh,'k')
        ...
        H.mainLine.DisplayName = 'Post Conditioning 1'; %strrep(exper_cond,'_',' ');
        keep_legend{1} = H.mainLine;
        h(2) = H.mainLine;
        yline(thresh,'b')
        hold off
        box on
        ...
        legend(h([1,2]),'Location','southoutside')
        ...
        thresh = max(calculateCI(reshape(exper_data.(exper_cond).trigger_data.pupil.data(:,plot_time < 0),1,[])));
        H = shadedErrorBar(plot_time(2:end), plot_data(1:end-1), plot_CI(1:end-1),'lineProps',{'k'});
        H.mainLine.DisplayName = 'Extinction';%strrep(exper_cond,'_',' ');
        h(1) = H.mainLine;
        yline(thresh,'k')
        ...
        thresh = max(calculateCI(reshape(exper_data.(exper_cond).trigger_data.pupil.data(:,plot_time < 0),1,[])));
        H = shadedErrorBar(plot_time(2:end), plot_data(1:end-1), plot_CI(1:end-1),'lineProps',{'b'});
        H.mainLine.DisplayName = 'Post Conditioning 2'; %strrep(exper_cond,'_',' ');
        ...
        uniformFormat(fig)
        if save_it
            savefig(fig,fullfile(plot_path,'2_evoked_pupil.fig'))
            exportgraphics(fig,fullfile(plot_path,'2_evoked_pupil.pdf'),'Resolution',300,'ContentType','vector')
        end
