% run_evoked.m
%
% Script that plots firing rate vs. pupil comparing conditions
%
% MATLAB R2021b
% Macauley Breault
% Created: 08-16-2022
...
for mouse = 1:numel(mouse_ids)
    ...
    for d = 1:numel(dates)
        ...
        %plot_CI = step_plot(CI(exper_data.(exper_cond).trigger_data.neural.data));
        ...
        legend('Location','northwest','Orientation','vertical','Color','none','EdgeColor','none')
        set(gca,'UserData','neural')
        ...
        % Save
        sameLimits(findobj(fig,'UserData','neural'),'y')
        sameLimits(findobj(fig,'UserData','pupil'),'y')
        sameLimits(fig,'x')
        uniformFormat(fig)
        if save_it
            savefig(fig,fullfile(plot_path,'2_evoked.fig'))
            exportgraphics(fig,fullfile(plot_path,'2_evoked.pdf'),'Resolution',300,'ContentType','vector')
        end
